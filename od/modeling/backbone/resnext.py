...
def _resnext(arch, block, layers, pretrained, **kwargs):
    ...
    if pretrained:
        model.load_state_dict(torch.hub.load_state_dict_from_url(model_urls[arch], progress=False),
                              strict=False)
    return model

@registry.BACKBONES.register('resnext50')
def resnext50_32x4d(cfg,pretrained=False, **kwargs):
...
@registry.BACKBONES.register('resnext101')
def resnext101_32x8d(cfg,pretrained=False, **kwargs):

...
from od.modeling.backbone.efficient_net.utils import (relu_fn, round_filters, round_repeats,
                                                      drop_connect, Conv2dSamePadding,
                                                      get_model_params, efficientnet_params,
...
INDICES = {
    ...
    'efficientnet-b6': [14, 30, 44],

    'efficientnet-b7': [17, 37, 54],
...
EXTRAS = {
    ...
    'efficientnet-b2': [
        # in,  out, k, s, p
        [(352, 128, 1, 1, 0), (128, 256, 3, 2, 1)],  # 5 x 5
        [(256, 128, 1, 1, 0), (128, 256, 3, 1, 0)],  # 3 x 3
        [(256, 128, 1, 1, 0), (128, 256, 3, 1, 0)],  # 1 x 1
    ...
    'efficientnet-b4': [
        # in,  out, k, s, p
        [(448, 128, 1, 1, 0), (128, 256, 3, 2, 1)],  # 5 x 5
        [(256, 128, 1, 1, 0), (128, 256, 3, 1, 0)],  # 3 x 3
        [(256, 128, 1, 1, 0), (128, 256, 3, 1, 0)],  # 1 x 1
    ...
    'efficientnet-b5': [
        # in,  out, k, s, p
        [(512, 128, 1, 1, 0), (128, 256, 3, 2, 1)],  # 5 x 5
        [(256, 128, 1, 1, 0), (128, 256, 3, 1, 0)],  # 3 x 3
        [(256, 128, 1, 1, 0), (128, 256, 3, 1, 0)],  # 1 x 1
    ...
    'efficientnet-b6': [
        # in,  out, k, s, p
        [(576, 128, 1, 1, 0), (128, 256, 3, 2, 1)],  # 5 x 5
        [(256, 128, 1, 1, 0), (128, 256, 3, 1, 0)],  # 3 x 3
        [(256, 128, 1, 1, 0), (128, 256, 3, 1, 0)],  # 1 x 1
    ...
    'efficientnet-b7': [
        # in,  out, k, s, p
        [(640, 128, 1, 1, 0), (128, 256, 3, 2, 1)],  # 5 x 5
        [(256, 128, 1, 1, 0), (128, 256, 3, 1, 0)],  # 3 x 3
        [(256, 128, 1, 1, 0), (128, 256, 3, 1, 0)],  # 1 x 1
...
class EfficientNet(nn.Module):
    ...
    def __init__(self, model_name, blocks_args=None, global_params=None, enable_extra=False):
        super().__init__()
        ...
        for block_args in self._blocks_args:
            ...
            self._blocks_args[count] = block_args._replace(
    ...
    @classmethod
    def from_name(cls, model_name, override_params=None, enable_extra=False):
        cls._check_model_name_is_valid(model_name)
        blocks_args, global_params = get_model_params(model_name, override_params)
        return EfficientNet(model_name, blocks_args, global_params, enable_extra=enable_extra)

    @classmethod
    def from_pretrained(cls, model_name, enable_extra=False):
        model = EfficientNet.from_name(model_name, enable_extra=enable_extra)

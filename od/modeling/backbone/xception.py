...
class ExtraLayers(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.avgpool(out2_2)
        return out1_2, out2_2, out
...
class Xception(nn.Module):
    ...
    def __init__(self, enable_extra=False, num_classes=1000):
        """ Constructor
        Args:
        ...
        self.avg = ExtraLayers(2048) if enable_extra else nn.AvgPool2d(3, stride=1)
    ...
    def forward(self, x):
        ...
        if enable_extra:
            channel: (32, 128, 256, 728, 2048, 512, 512, 512)
            output : (255, 127, 64, 32, 16, 8, 4, 2)
        else :
            channel: (32, 128, 256, 728, 2048, 512)
            output : (255, 127, 64, 32, 16, 8)
        """
        if isinstance(self.avg, nn.AvgPool2d):
            out5 = self.avg(x)
            return out0, out1, out2, out3, out4, out5
        else:
            out5, out6, out7 = self.avg(x)
            return out0, out1, out2, out3, out4, out5, out6, out7


@registry.BACKBONES.register('xception')
def xception(cfg,pretrained=False, **kwargs):
    """
    Construct Xception.
    ...
    if enable_extra:
        ...
        output : (255, 127, 64, 32, 16, 8)
    """
    model = Xception(enable_extra=cfg.MODEL.BACKBONE.EXTRA, **kwargs)
    if pretrained:
        model.load_state_dict(torch.hub.load_state_dict_from_url(model_urls['xception'], progress=False),
                              strict=False)
    return model

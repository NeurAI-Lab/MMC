...
class PostBackboneV1(nn.Module):
    def __init__(self,patch_size,inplanes,upsample,postbackbone):
        super().__init__()
        self.patch_size=patch_size
        self.conv=ExtraLayers(inplanes,upsample)
        self.postbackbone=postbackbone
    def forward(self, x):
        if self.postbackbone=="v1":
            x=x[0]
        elif self.postbackbone=="v3":
            x=x[-2]

        batch=x.shape[0]
        x=x.reshape((batch, self.patch_size, self.patch_size, -1))
        x=x.permute(0,3,1,2)
        x=self.conv(x)
        return x
...
class ExtraLayers(nn.Module):
    ...
    def __init__(self, inplanes,upsample=""):
        ...
        self.convbnrelu4_1 = ConvBnReluLayer(512, 256, kernel_size=1, padding=0, stride=1)
        self.convbnrelu4_2 = ConvBnReluLayer(256, 512, kernel_size=3, padding=1, stride=2)
    ...
    def forward(self, x):
        ...
        out4_1 = self.convbnrelu3_1(out3_2)
        out4_2 = self.convbnrelu3_2(out4_1)
        outs.append(out4_2)

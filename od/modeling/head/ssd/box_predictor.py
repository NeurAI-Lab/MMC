...
from od.layers import SeparableConv2d
from od.modeling.head.ssd import registry
...
class BoxPredictor(nn.Module):
    ...
    def forward(self, features):
        ...
        cls_logits = torch.cat([c.view(batch_size, -1) for c in cls_logits], dim=1).view(batch_size, -1, self.cfg.MODEL.NUM_CLASSES)
        bbox_pred = torch.cat([l.view(batch_size, -1) for l in bbox_pred], dim=1).view(batch_size, -1, 4)
